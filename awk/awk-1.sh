...
#awk '{print NR "-",$1=$3, $2=$5}' $data_file > "${data_file%%.*}".out   # extracts data from column three and five and prints with default one caharacter-gap between the columns for all lines with line numbers separated with "-"

#awk '{print NR "-",$1=$3, '\t', '\t', '\t', '\t', '\t', '\t', $2=$5}' $data_file > "${data_file%%.*}".out   # extracts data from column three and five and prints with 7 character-gaps but no column is empty between the columns for all lines with line numbers separated with "-"

#awk '{print $1=$3, $2="", $3="", $4="", $5=$5}' $data_file > "${data_file%%.*}".out   # extracts data from column three and five and prints with 4 character-gaps for all lines
...
#awk '{if($2 == 300) print $0}' $data_file > 300.out # extracts all the lines or rows having the data '300' in the 2nd column 
